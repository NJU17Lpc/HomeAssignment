...
public class Chapter3 {
     ...
     * Number类提供coefficient和index
     *
     *一共两种情况，
     *  1、index相同时，coefficient相加，保存到目标链表中。两个表中的Node同时被next覆盖
     *  2、index不同时，取index小的node，将他保存在新链表中，最后该node被next覆盖
     * 结束后
     *  1、可能两个链表全空，则返回目标链表
     *  2、有一个不为空，将这个链表后续node直接连接到目标链表后面
     *
     * 返回目标链表
     *
     * 输入：需要手动new节点类再add，在测试用例中给出
     * 输出：3.0*X^0-1.0*X^1-5.0*X^3+6.0*X^4+0.0*X^8+2.0*X^12
     *
     * @param list1
     * @param list2
     * @return
     ...
     * 遇到0时当作无效位，继续对数组下一位进行同样的操作
     * 遍历的时候只要忽略0就行了，多的一个if判断操作，且if的后续多余操作可忽略不计
     * 当整个数组中元素只剩一个，即size退化为1后，最后的元素就是要返回的元素
     *
